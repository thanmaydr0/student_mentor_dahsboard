      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 pb-10">
        ...
        >
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-4">
            <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Community & Messaging</h1>
            ...
            <Card className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">{selectedStudents.size} of {students?.length || 0} selected</p>
                </div>
                <button 
                  onClick={toggleAll}
                  ...
                  students?.map(student => (
                    ...
                    >
                      <div className={cn(
                        ...
                        selectedStudents.has(student.id) ? "border-indigo-600 bg-indigo-600 text-white" : "border-slate-300 bg-white"
                      )}>
                        {selectedStudents.has(student.id) && <CheckCircle2 size={14} />}
                      </div>
                      <div>
                        <p className={cn("text-sm font-medium", selectedStudents.has(student.id) ? "text-indigo-900" : "text-slate-700")}>
                          {student.full_name}
                        </p>
                        <p className="text-xs text-slate-500">Sem {student.semester} • {student.branch}</p>
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="lg:col-span-7 flex flex-col h-[700px]">
            <Card className="flex-1 flex flex-col">
              ...
              <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {/* Channels */}
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Sending Channels</label>
                  <div className="flex gap-3">
                    <button
                      ...
                      className={cn(
                        ...
                        channels.email ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                      )}
                    >
                      <Mail size={16} /> Email
                    </button>
                    <button
                      ...
                      className={cn(
                        "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-sm font-medium transition-colors",
                        channels.whatsapp ? "bg-green-600 border-green-600 text-white" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-sm font-medium transition-colors",
                        channels.sms ? "bg-sky-500 border-sky-500 text-white" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                      )}
                    >
                ...
                <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4 space-y-3">
                  ...
                  <div className="my-2 border-t border-indigo-100/60"></div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[160px] rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-shadow resize-y"
                  />
                </div>
