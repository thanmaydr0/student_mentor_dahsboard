    ...
    >
      <Card className="flex flex-col gap-5 sm:flex-row sm:items-center">
        <Avatar
          ...
          size="lg"
          userId={profile.id}
          className="ring-4 ring-brand-50 dark:ring-white/10"
        />

        <div className="flex flex-1 flex-col gap-2">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="text-xl font-semibold text-brand-900 dark:text-white">{profile.full_name}</h2>
            <Badge variant="info">Student</Badge>
          </div>

          <div className="flex flex-wrap items-center text-sm font-medium text-brand-500 dark:text-slate-400">
            <span>{profile.branch}</span>
            <span className="mx-2 h-1 w-1 rounded-full bg-slate-300 dark:bg-slate-600" />
            <span>Semester {profile.semester}</span>
          </div>

          <div className="mt-1 flex items-center gap-1.5 text-sm text-brand-600 dark:text-slate-400">
            <UserCircle size={16} className="text-brand-400 dark:text-brand-500" />
            <span className="font-medium text-brand-500 dark:text-slate-500">Mentor:</span>
            {mentorLoading ? (
