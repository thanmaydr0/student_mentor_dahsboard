...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: any[]; label?: string }) => {
  if (active && payload && payload.length > 0) {
    ...
    return (
      <div className="rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-3 shadow-lg dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)]">
        <p className="mb-2 font-semibold text-brand-900 dark:text-white">{data.subject_name}</p>
        <div className="flex flex-col gap-1 text-sm text-brand-600 dark:text-slate-400">
          <p>
            <span className="font-medium text-brand-500 dark:text-slate-500">Attendance:</span>{' '}
            <span
              className="font-bold"
          ...
          <p>
            <span className="font-medium text-brand-500 dark:text-slate-500">Classes:</span>{' '}
...
export default function AttendanceChart({ data, isLoading }: AttendanceChartProps) {
  ...
  const actionLink = (
    <Link
      ...
      className="text-sm font-medium text-brand-600 dark:text-amber-500 hover:text-brand-800 dark:hover:text-amber-400 hover:underline"
    >
      View details
    </Link>
  )
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
      <Card title="Attendance Overview" action={actionLink}>
        {/* Warning Banner */}
        {failingCount > 0 && (
          <div className="mb-4 mt-2 flex items-center gap-2 rounded-lg border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/40 p-3 text-sm text-red-700 dark:text-red-400">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                ...
                axisLine={false}
                tickLine={false}
                tick={{ fontSize: 12, fill: theme === 'dark' ? '#64748b' : '#94a3b8' }}
