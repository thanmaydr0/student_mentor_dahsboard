...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user && profile) {
      const dashPath = profile.role === 'mentor' 
        ? '/mentor/dashboard' 
        : profile.role === 'parent' 
          ? '/parent/dashboard' 
          : '/student/dashboard'
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-surface-50 via-brand-50 to-surface-100 dark:from-slate-950 dark:via-brand-900 dark:to-slate-900 px-4">
      ...
      >
        <div className="rounded-2xl border border-brand-100 dark:border-brand-800 bg-white/80 dark:bg-slate-900/80 p-8 shadow-card backdrop-blur-sm sm:p-10">
          {/* Logo */}
          <div className="mb-8 flex flex-col items-center gap-2">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-800 dark:bg-brand-700">
          ...
          <p className="mt-6 text-center text-sm text-brand-400 dark:text-slate-400">
            ...
            <Link
              to="/auth/signup"
              className="font-medium text-brand-700 dark:text-amber-400 underline-offset-2 transition-colors hover:text-brand-900 dark:hover:text-amber-300 hover:underline"
            >
              Create one
            </Link>
          </p>
