      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm font-medium text-brand-500 dark:text-slate-400">
              {currentDate}
            </p>
          </div>
        ...
        {showBanner && (
          <motion.div variants={itemVariants}>
            <div className="rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/40 p-5">
              <div className="flex items-start gap-3">
                {loadingTooLong && anyLoading ? (
                  <Clock size={20} className="mt-0.5 shrink-0 text-amber-600 dark:text-amber-500" />
                ) : (
                  <AlertCircle size={20} className="mt-0.5 shrink-0 text-amber-600 dark:text-amber-500" />
                )}
                <div className="flex-1 min-w-0">
                  ...
                  {hasErrors && (
                    <div className="mb-3 space-y-1">
                      {profileQuery.error && <p className="text-xs text-red-600 dark:text-red-400 font-mono">Profile: {profileQuery.error.message}</p>}
                      {attendanceQuery.error && <p className="text-xs text-red-600 dark:text-red-400 font-mono">Attendance: {attendanceQuery.error.message}</p>}
                      {gradesQuery.error && <p className="text-xs text-red-600 dark:text-red-400 font-mono">Grades: {gradesQuery.error.message}</p>}
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 rounded-lg border border-amber-300 dark:border-amber-900/50 bg-white dark:bg-amber-950/40 px-4 py-2 text-sm font-medium text-amber-700 dark:text-amber-400 shadow-sm transition-colors hover:bg-amber-50 dark:hover:bg-amber-900/50"
                    >
                      <RefreshCcw size={16} />
                      Reload Page
                    </button>
        ...
        <motion.div variants={itemVariants} className="flex justify-center pb-8 pt-2">
           <p className="text-xs text-brand-400 dark:text-slate-500 font-medium">Last updated {lastUpdated}</p>
        </motion.div>
